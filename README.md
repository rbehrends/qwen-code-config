...
Built-in provider presets currently include OpenRouter, Command Code (with separate OpenAI-compatible and Anthropic-compatible choices), OpenCode Go, OpenCode Zen, Kilo Code, NVIDIA, Ollama, Ollama Cloud, and LM Studio. Command Code catalogs are fetched from its keyless `/provider/v1/models` endpoint; use `CMD_API_KEY` for requests. The OpenAI-compatible catalog excludes Claude models, while the Anthropic-compatible catalog contains Claude models. Both choices display `Command Code` as the provider on configured model entries. Models added through the OpenCode Go and OpenCode Zen presets include the required dynamic `x-opencode-session` header, and the editor enables `outboundCorrelation.allowDynamicHeaderValues` when adding one.
